...
func setupDispatcherTest(t *testing.T, upstream string, logger *slog.Logger) (*DNSDispatcher, *MockGeoIpLookup, *blocklist.BlockList, *slog.Logger) {
	...
	dnsClient, err := NewRoundRobinClient(metrics, 2*time.Second, 2*time.Second, 1, logger, upstream)
	require.NoError(t, err)
...
func TestDNSDispatcher_NegativeCacheTtlFloor(t *testing.T) {
	...
	dnsClient, err := NewRoundRobinClient(metrics, 2*time.Second, 2*time.Second, 1, logger, "8.8.8.8:53")
	assert.NoError(t, err)
